...
function update()
  ...
  then
    -- no_spin_kit has changed due to command or button press
    local alt_agl_ft = get(DATAREF_ALT_AGL_FT)
    
    -- if airborn then overwrite no_spin_kit change back to prior value and return
    if alt_agl_ft > 10
    then
      set(DATAREF_NO_SPIN_KIT, prev_no_spin_kit)
      return
    end

    -- ok here we can toggle the spin kit weight
